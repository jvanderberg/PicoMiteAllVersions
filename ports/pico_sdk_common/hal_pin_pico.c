...
void hal_pin_set_drive_mA(uint32_t gpio, uint8_t mA) {
    ...
    if (mA <= 2)
        s = GPIO_DRIVE_STRENGTH_2MA;
    else if (mA <= 4)
        s = GPIO_DRIVE_STRENGTH_4MA;
    else if (mA <= 8)
        s = GPIO_DRIVE_STRENGTH_8MA;
    else
        s = GPIO_DRIVE_STRENGTH_12MA;
    gpio_set_drive_strength(gpio, s);
}

void hal_pin_set_pulls(uint32_t gpio, hal_pin_pull_t pull) {
    switch (pull) {
    case HAL_PIN_PULL_NONE:
        gpio_set_pulls(gpio, false, false);
        break;
    case HAL_PIN_PULL_UP:
        gpio_set_pulls(gpio, true, false);
        break;
    case HAL_PIN_PULL_DOWN:
        gpio_set_pulls(gpio, false, true);
        break;
    }
}
...
void hal_pin_set_function(uint32_t gpio, hal_pin_func_t func) {
    int sdk;
    switch (func) {
    case HAL_PIN_FUNC_NONE:
        sdk = GPIO_FUNC_NULL;
        break;
    case HAL_PIN_FUNC_SIO:
        sdk = GPIO_FUNC_SIO;
        break;
    case HAL_PIN_FUNC_UART:
        sdk = GPIO_FUNC_UART;
        break;
    case HAL_PIN_FUNC_I2C:
        sdk = GPIO_FUNC_I2C;
        break;
    case HAL_PIN_FUNC_SPI:
        sdk = GPIO_FUNC_SPI;
        break;
    case HAL_PIN_FUNC_PWM:
        sdk = GPIO_FUNC_PWM;
        break;
    case HAL_PIN_FUNC_PIO0:
        sdk = GPIO_FUNC_PIO0;
        break;
    case HAL_PIN_FUNC_PIO1:
        sdk = GPIO_FUNC_PIO1;
        break;
#ifdef GPIO_FUNC_PIO2
    case HAL_PIN_FUNC_PIO2:
        sdk = GPIO_FUNC_PIO2;
        break;
#endif
    default:
