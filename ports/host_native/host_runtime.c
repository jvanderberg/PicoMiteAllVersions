...
/* -1 = no PWM slice claimed. The shared PWM syscall rejects a channel
 * that equals a feature-owned slice; host has no such hardware, so the
 * sentinel keeps channel 0 usable for the BASIC PWM command. */
