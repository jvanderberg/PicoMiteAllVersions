...
static void audio_teardown_channel(void) {
    if (!s_tx) return;
...
static int option_i2s_ws_pin(void) {
    if (!audio_pin_invalid(ESP32_OPTION_AUDIO_I2S_WS))
        return ESP32_OPTION_AUDIO_I2S_WS;
    if (audio_pin_invalid(Option.audio_i2s_bclk)) return 0;
    int ws_gpio = PinDef[Option.audio_i2s_bclk].GPno + 1;
    if (ws_gpio < 0 || ws_gpio >= HAL_PORT_GPIO_COUNT) return 0;
    return codemap(ws_gpio);
}

static int option_i2s_mclk_pin(void) {
    return audio_pin_invalid(ESP32_OPTION_AUDIO_I2S_MCLK)
               ? 0
...
static audio_backend_t select_backend(void) {
    ...
    if ((ESP32_OPTION_AUDIO_KIND == ESP32_AUDIO_KIND_I2S ||
         ESP32_OPTION_AUDIO_KIND == ESP32_AUDIO_KIND_OFF) &&
