...
static void esp32_console_stdout_flush(void) {
    fflush(stdout);
    if (OptionConsole & 2) esp32_ili9341_lcd_flush_pending();
}

static const mm_runtime_console_adapter esp32_console_adapter = {
    .name = "esp32",
    .stdout_flush = esp32_console_stdout_flush,
};

void esp32_mmbasic_console_glue_init(void) {
    mmbasic_runtime_console_set_adapter(&esp32_console_adapter);
}
