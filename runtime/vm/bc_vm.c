...
static inline BCValue bc_vm_coerce_arg_value(BCValue value, uint8_t from_type, uint8_t to_type) {
    ...
    if (to_type == T_INT) {
        if (from_type == T_NBR)
            out.i = (int64_t)value.f;
        else if (from_type == T_STR)
            out.i = 0;
    } else if (to_type == T_NBR) {
        if (from_type == T_INT)
            out.f = (MMFLOAT)value.i;
        else if (from_type == T_STR)
            out.f = 0;
    }
...
static void bc_vm_execute_syscall(BCVMState * vm, uint16_t sysid, uint8_t argc,
    ...
    case BC_SYS_FILE_KILL: {
        ...
        if (sysid == BC_SYS_FILE_MKDIR)
            vm_sys_file_mkdir(path);
        else if (sysid == BC_SYS_FILE_CHDIR)
            vm_sys_file_chdir(path);
        else if (sysid == BC_SYS_FILE_RMDIR)
            vm_sys_file_rmdir(path);
        else
            vm_sys_file_kill(path);
        return;
    }
    ...
    if (sysid == BC_SYS_GFX_BOX || sysid == BC_SYS_GFX_RBOX) {
        ...
        if (sysid == BC_SYS_GFX_BOX) {
            vm_sys_graphics_box_execute((bc_vm_box_is_array_kind(kinds[0]) && bc_vm_box_is_array_kind(kinds[1])) ? GFX_BOX_MODE_VECTOR : GFX_BOX_MODE_SCALAR,
    ...
    if (sysid == BC_SYS_GFX_TRIANGLE || sysid == BC_SYS_GFX_POLYGON || sysid == BC_SYS_GFX_CIRCLE ||
        ...
        if (sysid == BC_SYS_GFX_TRIANGLE) {
            ...
            vm_sys_graphics_circle_execute((bc_vm_box_is_array_kind(kinds[0]) &&
                                            bc_vm_box_is_array_kind(kinds[1]) &&
                                            bc_vm_box_is_array_kind(kinds[2]))
                                               ? GFX_CIRCLE_MODE_VECTOR
                                               : GFX_CIRCLE_MODE_SCALAR,
                                           args, field_count, &errors);
            if (Option.Refresh) Display_Refresh();
            return;
            ...
            vm_sys_graphics_line_execute((bc_vm_box_is_array_kind(kinds[0]) &&
                                          ...
                                          bc_vm_box_is_array_kind(kinds[2]) &&
                                          bc_vm_box_is_array_kind(kinds[3]))
                                             ? GFX_LINE_MODE_VECTOR
                                             : GFX_LINE_MODE_SCALAR,
                                         args, field_count, &errors);
            return;
        } else {
            uint8_t field_count = *p++;
            ...
            vm_sys_graphics_pixel_execute((bc_vm_box_is_array_kind(kinds[0]) && bc_vm_box_is_array_kind(kinds[1])) ? GFX_PIXEL_MODE_VECTOR : GFX_PIXEL_MODE_SCALAR,
                                          args, field_count, &errors);
            if (Option.Refresh) Display_Refresh();
            return;
        }
...
#define POP_NUMERIC_I() \
    ((vm->sp < 0) ? (bc_vm_error(vm, "Stack underflow"), (int64_t)0) : (vm->stack_types[vm->sp] == T_NBR ? ({ MMFLOAT _x = vm->stack[vm->sp--].f; (_x >= 0) ? (int64_t)(_x + 0.5) : (int64_t)(_x - 0.5); }) : POP_I()))
#define POP_NUMERIC_F() \
    ((vm->sp < 0) ? (bc_vm_error(vm, "Stack underflow"), (MMFLOAT)0) : (vm->stack_types[vm->sp] == T_NBR ? POP_F() : (MMFLOAT)POP_I()))
...
op_jcmp_i: {
    ...
    case BC_JCMP_GE:
        ...
        break;
    default:
        bc_vm_error(vm, "Invalid JCMP_I relation %u", (unsigned)rel);
    }
    if (take) {
        if (offset < 0) bc_vm_poll_interrupts();
        vm->pc += offset;
...
op_jcmp_f: {
    ...
    case BC_JCMP_GE:
        ...
        break;
    default:
        bc_vm_error(vm, "Invalid JCMP_F relation %u", (unsigned)rel);
    }
    if (take) {
        if (offset < 0) bc_vm_poll_interrupts();
        vm->pc += offset;
...
op_math_sgn: {
    ...
    if (vm->stack_types[vm->sp] == T_INT) {
        int64_t v = vm->stack[vm->sp].i;
        vm->stack[vm->sp].i = (v > 0) ? 1 : (v < 0) ? -1
                                                    : 0;
    } else {
        MMFLOAT v = vm->stack[vm->sp].f;
        vm->stack[vm->sp].i = (v > 0.0) ? 1 : (v < 0.0) ? -1
                                                        : 0;
        vm->stack_types[vm->sp] = T_INT;
    }
    DISPATCH();
...
op_file: {
    ...
    case BC_FILE_KILL: {
        ...
        memcpy(path, s + 1, len);
        path[len] = '\0';
        if (subop == BC_FILE_MKDIR)
            vm_sys_file_mkdir(path);
        else if (subop == BC_FILE_CHDIR)
            vm_sys_file_chdir(path);
        else if (subop == BC_FILE_RMDIR)
            vm_sys_file_rmdir(path);
        else
            vm_sys_file_kill(path);
        break;
