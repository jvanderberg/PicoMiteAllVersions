...
| [11 — sweep](real-hal/phases-8-to-13.md#phase-11--sweep--remaining-drivers--scope-cleanup) | ✅ | bc_runtime.c, bc_bridge.c, vm_sys_graphics.c, MMBasic.c promoted to STRICT_FILES (steps 12/13/14/20). All 12 scored core files at zero target/port-config ifdefs and zero runtime HAL_PORT_* folds; the remaining `#ifdef`s in MMBasic.c (2) and FileIO.c (2) are permitted feature-flag (GUICONTROLS / MMFAMILY / __PIC32MX__) and `min/max` polyfill gates. Optional driverisations (`drivers/watchdog_pico/`, `drivers/gps_uart/`, `drivers/goodix_touch/`, `drivers/mouse_serial/`) deferred — purity goal is met without them. CFunctions architectural decision deferred to Phase 13. |
