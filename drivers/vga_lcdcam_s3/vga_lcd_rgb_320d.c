...
/*
 * [320d] This file is a fork of esp-idf's esp_lcd_panel_rgb.c
 * (components/esp_lcd/rgb/, Apache-2.0) with one structural change:
 * each frame-buffer row is mounted on TWO consecutive DMA descriptors,
 * so the panel emits every row twice (hardware line doubling). With the
 * horizontal timing halved (320 active pixels at ~12.59 MHz) the
 * monitor still sees a standard 31.47 kHz / 60 Hz signal, but the
 * frame buffer is only h_res x v_res/2 bytes — small enough for
 * internal SRAM, scanned out by GDMA with no CPU in the path.
 *
 * Every divergence from the upstream file is marked with a [320d]
 * comment. The public symbols are renamed below so the unmodified
 * esp_lcd component (still used for the 640x480 console mode) links
 * alongside this fork.
 */
#define VGA320D_LINE_REPEAT 2

/* [320d] Optional boot-time frame-buffer reservation, set by the port
 * before the first panel create. A 76.8 KB internal-DMA block may be
 * unallocatable once the heap warms up, so the port grabs it at boot
 * and the panel reuses it across rebuilds (mode switches) instead of
 * allocating and freeing its own. Only adopted when the panel's fb size
...
#define esp_lcd_rgb_panel_restart vga320d_rgb_panel_restart
#define esp_lcd_rgb_panel_get_frame_buffer vga320d_rgb_panel_get_frame_buffer
#define esp_lcd_rgb_panel_refresh vga320d_rgb_panel_refresh
#define esp_lcd_rgb_panel_set_yuv_conversion vga320d_rgb_panel_set_yuv_conversion
...
static esp_err_t lcd_rgb_panel_alloc_frame_buffers(esp_rgb_panel_t *rgb_panel)
{
    ...
    for (int i = 0; i < rgb_panel->num_fbs; i++) {
        if (fb_in_psram) {
            ...
            /* [320d] prefer the boot-reserved buffer (see above). */
            if (i == 0 && vga320d_reserved_fb && rgb_panel->fb_size == vga320d_reserved_fb_size) {
                rgb_panel->fbs[i] = vga320d_reserved_fb;
            } else {
    ...
    if (rgb_panel->bb_size) {
        /* [320d] a bounce-mode panel never coexists with the reserved-fb
         * native panel, so the idle reservation doubles as the bounce
         * arena: with most of the internal heap pre-reserved there is no
         * room left to allocate bounce buffers the normal way. */
        bool bb_from_reserved = vga320d_reserved_fb &&
                                RGB_LCD_PANEL_BOUNCE_BUF_NUM * rgb_panel->bb_size <= vga320d_reserved_fb_size;
        for (int i = 0; i < RGB_LCD_PANEL_BOUNCE_BUF_NUM; i++) {
            // bounce buffer must be allocated from internal memory for performance
            if (bb_from_reserved) {
                rgb_panel->bounce_buffer[i] = vga320d_reserved_fb + i * rgb_panel->bb_size;
                memset(rgb_panel->bounce_buffer[i], 0, rgb_panel->bb_size);
            } else {
...
static esp_err_t lcd_rgb_panel_destroy(esp_rgb_panel_t *rgb_panel)
{
    ...
    for (int i = 0; i < RGB_LCD_PANEL_BOUNCE_BUF_NUM; i++) {
        if (rgb_panel->bounce_buffer[i]) {
            /* [320d] bounce buffers carved from the boot reservation
             * outlive the panel (see lcd_rgb_panel_alloc_frame_buffers). */
            uint8_t *bb = (uint8_t *)rgb_panel->bounce_buffer[i];
            if (!(vga320d_reserved_fb && bb >= vga320d_reserved_fb &&
                  bb < vga320d_reserved_fb + vga320d_reserved_fb_size)) {
...
esp_err_t esp_lcd_new_rgb_panel(const esp_lcd_rgb_panel_config_t *rgb_panel_config, esp_lcd_panel_handle_t *ret_panel)
{
    ...
    /* [320d] line-doubled panels (no bounce buffers) scan each row
     * twice via paired descriptors, so their buffer holds half the
     * output lines. Bounce-mode panels are stock upstream. */
    ...
    if (bb_size == 0) fb_size /= VGA320D_LINE_REPEAT;
    ...
    /* [320d] timings must be set before the DMA link init: the doubled
     * descriptor mounting derives rows from timings.v_res (upstream
     * assigns timings later, but never reads it in init_trans_link). */
...
static esp_err_t rgb_panel_del(esp_lcd_panel_t *panel)
{
    ...
    /* [320d] quiesce before teardown. Upstream assumes the panel was
     * stopped before del, but the VGA mode switches delete a panel that
     * is still streaming: if the bounce-refill EOF interrupt fires
     * mid-destroy it operates on dying state (observed as an interrupt
     * watchdog panic inside the ISR's memcpy). Silence every interrupt
     * source and halt the engine first; on the owning core a pending
     * ISR then drains against still-intact state before destroy runs. */
    if (rgb_panel->intr) {
        esp_intr_disable(rgb_panel->intr);
    }
    lcd_ll_enable_auto_next_frame(rgb_panel->hal.dev, false);
    lcd_ll_stop(rgb_panel->hal.dev);
    if (rgb_panel->dma_chan) {
        gdma_stop(rgb_panel->dma_chan);
    }
...
/* [320d] not IRAM: this fork's panels always have a frame buffer and
 * no bounce buffers, so the bounce-fill path is never reached. */
static bool lcd_rgb_panel_fill_bounce_buffer(esp_rgb_panel_t *panel, uint8_t *buffer)
{
...
static esp_err_t lcd_rgb_panel_init_trans_link(esp_rgb_panel_t *rgb_panel)
{
    ...
    if (rgb_panel->bb_size) {
        ...
        /* [320d] one descriptor per OUTPUT line; consecutive descriptor
         * pairs point at the same frame-buffer row, so the hardware
         * emits each row VGA320D_LINE_REPEAT times. */
        uint32_t rows = rgb_panel->timings.v_res / VGA320D_LINE_REPEAT;
        size_t row_bytes = rgb_panel->fb_size / rows;
        uint32_t num_dma_nodes = rgb_panel->timings.v_res;
        gdma_link_list_config_t link_cfg = {
            .item_alignment = LCD_GDMA_DESCRIPTOR_ALIGN,
        ...
        /* [320d] transient CPU-only config array (~11.5 KB for 480 nodes):
         * prefer PSRAM so it does not compete with the descriptor link
         * list for the little internal RAM left after the scanout and
         * interpreter-stack carves. */
        gdma_buffer_mount_config_t *mount_cfgs = heap_caps_calloc(
            num_dma_nodes, sizeof(gdma_buffer_mount_config_t),
        ...
        for (size_t i = 0; i < rgb_panel->num_fbs; i++) {
            ...
            for (uint32_t n = 0; n < num_dma_nodes; n++) {
                ...
                mount_cfgs[n].length = row_bytes;
                mount_cfgs[n].buffer_alignment = buffer_alignment;
                mount_cfgs[n].flags.mark_eof = (n == num_dma_nodes - 1);
                mount_cfgs[n].flags.mark_final =
                    (n == num_dma_nodes - 1) && !rgb_panel->flags.stream_mode;
            }
            esp_err_t mount_err = gdma_link_mount_buffers(rgb_panel->dma_fb_links[i], 0, mount_cfgs,
                                                          num_dma_nodes, NULL);
            if (mount_err != ESP_OK) {
                free(mount_cfgs);
                ESP_RETURN_ON_ERROR(mount_err, TAG, "mount DMA frame buffer failed");
            }
        }
        free(mount_cfgs);
        // create restart link
        ...
        gdma_buffer_mount_config_t restart_buffer_mount_cfg = {
            ...
            .buffer_alignment = buffer_alignment,
            /* [320d] descriptor 0 of the ring covers one row, so the
             * restart node replays that row (minus the FIFO-preserved
             * bytes) before re-entering the ring at descriptor 1. */
            .length = row_bytes - restart_skip_bytes,
...
// time to reset DMA.
/* [320d] not IRAM: CONFIG_LCD_RGB_ISR_IRAM_SAFE is off, so the LCD
 * interrupt is masked while flash cache is down — flash-resident
 * handlers are fine and the duplicated IRAM is reclaimed. */
static void lcd_rgb_panel_try_restart_transmission(esp_rgb_panel_t *panel)
{
...
/* [320d] not IRAM: nothing calls vga320d_rgb_panel_set_pclk, so the
 * need_update_pclk branch in the ISR never takes this path. */
static void lcd_rgb_panel_try_update_pclk(esp_rgb_panel_t *rgb_panel)
{
    hal_utils_clk_div_t lcd_clk_div = {};
...
/* [320d] not IRAM: see lcd_rgb_panel_try_restart_transmission. */
static void rgb_lcd_default_isr_handler(void *args)
{
