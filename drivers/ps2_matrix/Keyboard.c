...
bool sendCommand(int cmd, int clock, int data) {
    ...
    uSec(2);
    while (PinRead(clock))
        if (InkeyTimer >= 500) { // wait for the keyboard to pull the clock low
            return false;        // wait for the keyboard to pull the clock low
        }
...
void processcode(unsigned char Code) {
    ...
    if (Code == 0xaa) { // self test code (a keyboard must have just been plugged in)
        ...
        if (!KeyE0 && Code == 0x83)
            ...
            switch (Option.KeyboardConfig) {                        // an international keycode pressed with
...
/* USB-host hooks for MM.USB / MM.USB VID / MM.USB PID. PS/2 builds
 * have no USB devices, so the hooks always return 0. The full
 * `volatile struct s_HID HID[4]` array (≈336 B BSS) lives only in the
